...
function FarmerDetailPage() {
    ...
    const fetchData = async () => {
        ...
        setError('');
        try {
            // Task 2.2: Refactored API calls
            const farmerResponse = await apiClient.get(`/api/farmers/${farmerId}`);
            const farmsResponse = await apiClient.get(`/api/farmers/${farmerId}/farms`);
            ...
            const farmsWithDetailsPromises = farmsData.map(async (farm) => {
                const [inspectionsRes, certificatesRes, fieldsRes] = await Promise.all([
                    apiClient.get(`/api/farms/${farm.id}/inspections`),
                    apiClient.get(`/api/farms/${farm.id}/certificates`),
                    apiClient.get(`/api/farms/${farm.id}/fields`),
                ]);
                return {
                    ...farm,
    ...
    const handleApprove = async (inspectionId) => {
        try {
            await apiClient.post(`/api/inspections/${inspectionId}/approve`);
    ...
    const handleSaveFarm = async (farmData) => {
        ...
        try {
            if (editingFarm) {
                await apiClient.put(`/api/farms/${editingFarm.id}`, dataToSend);
                toast.success('Farm updated successfully!');
            } else {
                await apiClient.post(`/api/farms`, dataToSend);
    ...
    const handleSaveField = async (fieldData) => {
        ...
        try {
            if (editingField) {
                await apiClient.put(`/api/fields/${editingField.id}`, dataToSend);
                toast.success('Field updated successfully!');
            } else {
                await apiClient.post(`/api/fields`, dataToSend);
